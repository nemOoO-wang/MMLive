...
- (IBAction)clickViewDetail:(id)sender {
    ...
    [self performSegueWithIdentifier:@"detail" sender:@{@"title":self.titleArr[gesture.view.tag-1], @"code":@(500+gesture.view.tag-1)}];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"detail"]) {
        FemaleRuleVC *destinVC = [segue destinationViewController];
        destinVC.navigationItem.title = sender[@"title"];
        destinVC.searchCode = sender[@"code"];
